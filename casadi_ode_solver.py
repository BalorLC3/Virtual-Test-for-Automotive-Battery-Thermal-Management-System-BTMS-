...
def battery_dynamics_ode_ca(state, controls, disturbances, params):
    ...
    # ca.fmax is equivalent to np.maximum
    ...
    # Avoid ghost consume of energy (when w < 0)
    ...
    # Quadratic Logic
    # ca.fmax(disc, 0) to avoid NaNs in the sqrt
    ...
    # IF SOC > 0.995 AND Corriente < 0 (Carga) -> I=0
    I_batt = ca.if_else(ca.logic_and(soc >= 0.995, I_batt < 0), 0.0, I_batt)
    # IF SOC < 0.005 AND Corriente > 0 (Descarga) -> I=0
    ...
    Cr = C_min / (C_max + 1e-9) # Avoid div/0
